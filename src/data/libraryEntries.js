export const libraryEntries = [
  {
    id: 'tree-of-life',
    title: 'Tree of Life',
    tags: ['tree', 'tree of life', 'sefirot', 'structure', 'pillars'],
    content:
      'The Tree of Life is a living architecture of relationship, descent, balance, and return. In LuminaNexus, it serves both as contemplative map and navigational order. The sefirot are not isolated ideas but presences held in relation.',
  },
  {
    id: 'tiferet',
    title: 'Tiferet',
    tags: ['tiferet', 'beauty', 'balance', 'harmony', 'heart'],
    content:
      'Tiferet is beauty, harmony, and the radiant balance point within the Tree. It is often understood as the heart of the structure and lives in relation to Chesed and Gevurah. It is not superficial beauty, but ordered beauty born from right relationship.',
  },
  {
    id: 'chesed',
    title: 'Chesed',
    tags: ['chesed', 'mercy', 'kindness', 'love', 'expansion'],
    content:
      'Chesed is mercy, generosity, and expansive lovingkindness. Within the Tree it represents outpouring, benevolence, and open-handed movement.',
  },
  {
    id: 'gevurah',
    title: 'Gevurah',
    tags: ['gevurah', 'strength', 'severity', 'judgment', 'restraint'],
    content:
      'Gevurah is strength, restraint, and judgment. It gives contour, seriousness, and limit. In right relation to Chesed, it helps form the conditions for harmony.',
  },
  {
    id: 'yesod',
    title: 'Yesod',
    tags: ['yesod', 'foundation', 'connection', 'channel'],
    content:
      'Yesod is foundation, transmission, and connection. In LuminaNexus it relates naturally to chavruta, exchange, and the movement of insight into communicable form.',
  },
  {
    id: 'malkhut',
    title: 'Malkhut',
    tags: ['malkhut', 'kingdom', 'presence', 'manifestation'],
    content:
      'Malkhut is manifestation, presence, and reception. It is where what is ordered above becomes visible below. In LuminaNexus it relates to what is embodied, shared, and lived.',
  },
  {
    id: 'keter',
    title: 'Keter',
    tags: ['keter', 'crown', 'will', 'source', 'beginning'],
    content:
      'Keter is crown, hidden beginning, and source-point above articulation. It is often approached through reverence, silence, and orientation more than by analysis alone.',
  },
  {
    id: 'hebrew-letters',
    title: 'Hebrew Letters',
    tags: ['hebrew', 'letters', 'language', 'symbol'],
    content:
      'In LuminaNexus, Hebrew letters are not treated merely as vocabulary but as form, sound, number, symbol, and pathway. Language becomes both study and architecture.',
  },
  {
    id: 'ivritcode',
    title: 'IvritCode',
    tags: ['ivritcode', 'code', 'symbolic architecture', 'hebrew computation'],
    content:
      'IvritCode is a symbolic framework in which Hebrew letters are approached as meaningful structural elements within a computational imagination. It belongs where technical precision meets contemplative meaning.',
  },
  {
    id: 'chavruta',
    title: 'Chavruta',
    tags: ['chavruta', 'study', 'paired study', 'dialogue', 'guidance'],
    content:
      'Chavruta is not mere answer delivery. It is a shared movement of study: asking, clarifying, locating, and returning. ChavrutaGPT should feel companion-like, reverent, and structured.',
  },
]
