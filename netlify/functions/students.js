constimport { getStore } from "@netlify/blobs";
...
function requireTeacherPin(event) {
  ...
  if (!expectedPin) {
    return {
      ok: false,
      response: json(500, {
        error: "Missing CHAVRUTA_TEACHER_PIN environment variable in Netlify.",
      }),
    };
  }
...
function sanitizeStudent(input, existing = null) {
  const now = new Date().toISOString();

  const id =
    existing && existing.id
      ? existing.id
      : cleanId(input.id) || makeId();
...
export async function handler(event) {
  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      const student = sanitizeStudent(body);
    ...
    return json(500, {
      error:
        error && error.message
          ? error.message
          : "Unexpected server error in students function.",
    });
  }
} STORimport { getStore } from "@netlify/blobs";
import { randomUUID } from "node:crypto";
